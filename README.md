...
- **RAG-based AI Assistant:** Natural language interface for querying complex patterns such as refund issues or support quality.
- **Interactive Dashboard:** Comprehensive Streamlit interface for visualizing high-level analytics and granular reports.
...
utils/                # Modular helper scripts (analytics_utils.py, rag_utils.py, retriever_utils.py)
...
Ensure that the `faiss_store/` directory contains the required index files. If missing, run:
...
* **Path Management:** Always utilize relative paths to ensure cross-platform compatibility.
